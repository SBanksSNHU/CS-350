...
class ManagedDisplay():
    ...
    def __init__(self):
        ...
        self.lcd_d5 = digitalio.DigitalInOut(board.D16)
        self.lcd_d6 = digitalio.DigitalInOut(board.D19)
    ...
    def updateLights(self):
        ...
        if(self.current_state.id == 'heat'):
            ...
            if(temp >= self.setPoint):
                # Temperature is already above the setPoint
                redLight.on()
            else:
                # Temperature is lower than setPoint, heat is on
                redLight.pulse()
        elif(self.current_state.id == 'cool'):
            # Thermostat is set to cool
            if(temp <= self.setPoint):
                # Temperature is already less than setpoint
                blueLight.on()
            else:
                # Temperature is above setPoint, cooling is on
                blueLight.pulse()
        else:
            # Thermostat is Off
            pass
